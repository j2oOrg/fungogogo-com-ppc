<?php
/**
 * Template Name: FAQ
 * FAQ page template.
 */
get_header();
?>

<main class="faq-wrap-j5b2">
  <section class="faq-hero-c8r1" id="faq">
    <div class="shell-wrap-z4m2">
      <p class="eyebrow-t8k9">Need answers?</p>
      <h1 class="section-title-m3y2" style="font-size:2.5rem; margin-bottom:0.35rem;">Frequently Asked Questions</h1>
      <p class="section-sub-p0c6" style="max-width:860px;">Answers to common questions about the fungogogo Raffle.</p>
      <p class="faq-meta-u6e4">Operator / Licensee: fungogogo Association Foundation • AGLC Raffle Licence #662961 • Draw: Feb 25, 2026 • 995 tickets at $10</p>
      <p class="section-sub-p0c6" style="max-width:860px;">fungogogo is the campaign/brand name only and is not the operator; all raffle operations are run by the fungogogo Association Foundation under AGLC oversight.</p>
    </div>
  </section>

  <section class="faq-body-m2c7" aria-labelledby="faq-list">
    <div class="shell-wrap-z4m2 faq-grid-r4x8">
      <article class="faq-card-n8k5">
        <h3>How much are tickets and how many are available?</h3>
        <p>Tickets are $10 each. There are only 995 tickets issued for this raffle.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>Who can buy a ticket?</h3>
        <p>You must be 18+ and physically located in Alberta, Canada. Employees and immediate family of the licensee are not eligible.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>Is there a limit to how many tickets I can purchase?</h3>
        <p>No. You can buy multiple tickets; each ticket increases your chances of winning.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>When and where is the draw?</h3>
        <p>The draw is scheduled for February 25, 2026 at Fort Edmonton Park, Edmonton. It will follow Alberta Gaming regulations.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>What is the prize?</h3>
        <p>One grand prize package valued at $3,780 retail. The prize is awarded as-is with no cash alternative and is not transferable.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>What are my odds of winning?</h3>
        <p>Maximum odds are 1 in 995, depending on total tickets sold.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>How will the winner be notified?</h3>
        <p>We will contact the winner within 5 business days using the information provided at purchase. The winner must respond within 30 days.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>Do I need to be present at the draw?</h3>
        <p>No, you do not need to be present. We will contact the winner directly and post the winner on our site.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>Are ticket sales refundable?</h3>
        <p>All ticket sales are final—no refunds or exchanges.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>Where do the proceeds go?</h3>
        <p>Proceeds support conservation-focused initiatives inspired by the Pheasant Release Program.</p>
      </article>

      <article class="faq-card-n8k5">
        <h3>What if I have more questions?</h3>
        <p>Reach out via <a href="mailto:support@fungogogo.com" class="link-accent-m8q5">support@fungogogo.com</a> or use the <a href="<?php echo esc_url( home_url('/#contact') ); ?>">contact section</a> on the homepage for assistance.</p>
      </article>
    </div>
  </section>
</main>

<?php get_footer(); ?>
